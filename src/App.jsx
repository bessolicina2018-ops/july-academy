...
function TeacherStudents({ data, refresh }) {
  ...
  const toggleIntensive = async (s) => { await db.setEnrolledIntensive(s.id, !s.enrolledIntensive); refresh(); };
              ...
              <Card key={s.id} className="p-4 flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-1.5 text-xs" style={{ color: MUTED }}>
                    <input type="checkbox" checked={s.enrolledIntensive} onChange={() => toggleIntensive(s)} /> Intensive course
                  </label>
              ...
              <div className="space-y-3">
                {doc.homework.map((h) => (
                  <div key={h.id} className="rounded-lg p-4" style={{ backgroundColor: CARD_BEIGE }}>
                    ...
                    <p className="text-xs mt-1" style={{ color: MUTED }}>{h.instructions}</p>
                    {h.submissionText && <p className="text-xs mt-2 italic" style={{ color: INK }}>"{h.submissionText}"</p>}
...
function TeacherIntensive({ data, refresh }) {
  const enrolled = data.students.filter((s) => s.enrolledIntensive);
  const [selected, setSelected] = useState(enrolled[0]?.id || "");
  const [showTask, setShowTask] = useState(false);
  const [taskForm, setTaskForm] = useState({ title: "", instructions: "" });
  const [docDraft, setDocDraft] = useState(data.intensiveCourse.generalDoc);

  const saveDoc = async () => { await db.saveGeneralDoc(docDraft); refresh(); };
  ...
  const studentTasks = data.intensiveCourse.students[selected]?.tasks || [];

  return (
    <div>
      <SectionTitle sub="A shared classroom document for everything covered in the intensive course, plus individual tasks per student.">Intensive course</SectionTitle>
      <Card className="p-5 mb-6">
        <h3 className="font-medium mb-2" style={{ fontFamily: "Georgia, serif" }}>General class document</h3>
      ...
      {enrolled.length === 0 ? <EmptyState text="No students enrolled in the intensive course yet — enroll them from Students & groups." /> : (
        <Card className="p-5">
            ...
            <div className="space-y-3">
              {studentTasks.map((t) => (
                <div key={t.id} className="rounded-lg p-4" style={{ backgroundColor: CARD_BEIGE }}>
                  ...
                  <p className="text-xs mt-1" style={{ color: MUTED }}>{t.instructions}</p>
                  {t.submissionText && <p className="text-xs mt-2 italic">"{t.submissionText}"</p>}
...
function StudentApp({ data, refresh, student, onLogout }) {
  ...
  const tabs = [
    ...
    ...(student?.enrolledIntensive ? [{ key: "intensive", label: "Intensive classroom", icon: GraduationCap }] : []),
          ...
          <div className="space-y-4">
            {doc.homework.map((h) => (
              <div key={h.id} className="rounded-lg p-4" style={{ backgroundColor: CARD_BEIGE }}>
                ...
                <p className="text-xs mt-1" style={{ color: MUTED }}>{h.instructions}</p>
                {h.status === "checked" ? (
                  <><p className="text-xs mt-2 italic">"{h.submissionText}"</p><Feedback text={h.aiFeedback} /></>
                ) : (
                  <>
...
function StudentIntensive({ data, refresh, student }) {
  const tasks = data.intensiveCourse.students[student.id]?.tasks || [];
  const [drafts, setDrafts] = useState({});
  const [loadingId, setLoadingId] = useState(null);
  const submit = async (taskId) => {
    const text = drafts[taskId];
    if (!text || !text.trim()) return;
    setLoadingId(taskId);
  ...
  return (
    <div>
      <SectionTitle sub="Everything from the intensive course, plus your own tasks.">Intensive classroom</SectionTitle>
      <Card className="p-5 mb-5"><h3 className="font-medium mb-2" style={{ fontFamily: "Georgia, serif" }}>General class document</h3><div className="text-sm whitespace-pre-line" style={{ color: data.intensiveCourse.generalDoc ? INK : MUTED }}>{data.intensiveCourse.generalDoc || "Nothing posted yet."}</div></Card>
      <Card className="p-5">
          ...
          <div className="space-y-4">
            {tasks.map((t) => (
              <div key={t.id} className="rounded-lg p-4" style={{ backgroundColor: CARD_BEIGE }}>
                ...
                <p className="text-xs mt-1" style={{ color: MUTED }}>{t.instructions}</p>
                {t.status === "checked" ? (<><p className="text-xs mt-2 italic">"{t.submissionText}"</p><Feedback text={t.aiFeedback} /></>) : (
                  <>
...
function StudentCourses({ data, refresh, student }) {
  ...
  const submit = async (taskId) => {
    const text = drafts[taskId];
    if (!text || !text.trim() || !course) return;
    setLoadingId(taskId);
                    ...
                    {course.tasks.map((t) => {
                      ...
                      return (
                        <div key={t.id} className="rounded-lg p-4" style={{ backgroundColor: CARD_BEIGE }}>
                          ...
                          <p className="text-xs mt-1" style={{ color: MUTED }}>{t.instructions}</p>
                          {sub ? (<><p className="text-xs mt-2 italic">"{sub.submissionText}"</p><Feedback text={sub.aiFeedback} /></>) : (
                            <>
